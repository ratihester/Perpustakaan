...
<body>
  <section id="container">
   ...
   <aside>
    ...
    <?php
      ...
      $sql = "SELECT * FROM `tbl_peminjaman` JOIN `tbl_anggota` ON `tbl_peminjaman`.`id_anggota` = `tbl_anggota`.`id_anggota` JOIN `tbl_buku` ON `tbl_buku`.`idbuku` = `tbl_peminjaman`.`idbuku` JOIN `tbl_kelas` ON `tbl_anggota`.`id_kelas` = `tbl_kelas`.`id_kelas` WHERE `tbl_peminjaman`.`status`=1 OR `tbl_peminjaman`.`status`=3 ";
      $result = mysqli_query($connection,$sql);
    ?>

     <section id="main-content">
      <section class="wrapper">
        <h3>Data Peminjam Mingguan</h3>
        <div class="row mb">
          <div>
         <a href="tahun_ajaran_peminjaman.php" class="btn btn-theme"><i class="fa fa-plus"></i> Data Peminjam</a>
          </div>
          <!-- page start-->
          ...
          <td>
            <form action="kembali.php" method="post">
              <input type="hidden" name="id_kembali" value="<?php echo $data['idpeminjaman']; ?>">
          ...
          <td>
            ...
            ?>
            <form action="kembali.php" method="post">
              <input type="hidden" name="id_panjang" value="<?php echo $data['idpeminjaman']; ?>">
              <input type="hidden" name="tgl_kembali" value="<?php echo $data['tgl_kembali']; ?>">
              <input type="submit" value="Perpanjangan">
            </form>
            <?php 
            }else{
              echo "batas perpanjangan 1x";
            }
            ?>
          </td> 
                 </tr>
  ...
  <div class="modal fade modalKembali" tabindex="-1" role="dialog" aria-labelledby="modalKembali" aria-hidden="true" id="modalKembali">
